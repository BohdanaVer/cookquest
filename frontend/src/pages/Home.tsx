...
export default function Home() {
  ...
  const profile = { username: 'Bohdana', xp: 850, level: 4, rating_score: 1200 };
  const totalCooked = 12;
  ...
  const pendingBattles = [
    {
      id: 'mock-battle-1',
      recipe: { name: 'Спагетті Карбонара' },
      challenger: { username: 'AlexChef' }
    }
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-center animate-slide-up">
        <MascotStatic name={activeMascot} mood="happy" size={100} message={`Вітаю, ${profile.username}!`} />
      </div>
      ...
      <div className="relative bg-gradient-to-br from-[#1a1a2e] to-[#16213e] rounded-3xl p-5 border border-white/5 animate-slide-up">
        <div className="flex items-center gap-4">
          <div className="relative flex-shrink-0">
            <svg width="100" height="100" className="rotate-[-90deg]">
              ...
              <circle
                ...
                strokeDasharray={circumference} strokeDashoffset={dashOffset} className="animate-xp-fill"
              />
            </svg>
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-extrabold text-white truncate">{profile.username}</h1>
            <p className="text-xs text-gray-400 font-medium">{levelInfo?.name || 'Досвідчений кухар'}</p>
            <div className="mt-2 flex items-center gap-1.5">
              <Zap size={12} className="text-green-400" />
              <div className="flex-1 bg-white/5 rounded-full h-2 overflow-hidden">
                <div className="bg-gradient-to-r from-green-500 to-green-400 h-full rounded-full transition-all" style={{ width: `${xpProgress}%` }} />
              </div>
              <span className="text-[10px] text-green-400 font-bold">{profile.xp} XP</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-4">
          ...
          <div className="bg-white/5 rounded-xl p-2.5 text-center">
            <p className="text-lg font-extrabold text-orange-400">{profile.rating_score}</p>
      ...
      {/* ЗАГОТОВКА 1: Конкретний щоденний квест */}
      {todayChallenge && (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">Your rating: #{profile.rating_score}</p>
          </div>
