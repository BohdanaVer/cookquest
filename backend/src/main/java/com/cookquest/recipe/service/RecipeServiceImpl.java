...
import com.cookquest.recipe.ai.client.GroqClient;
...
public class RecipeServiceImpl implements RecipeService {
    ...
    public RecipeListResponse generateRecipes(GenerateRecipeRequest request) {
        ...
        try {
            ...
            for (JsonNode node : validatedNodes) {
                RecipeItem item = objectMapper.convertValue(node, RecipeItem.class);
    ...
    @Override
    public VisionResponse recognizeIngredients(List<MultipartFile> files) {
        ...
        try {
            ...
            String prompt = visionPromptBuilder.buildIngredientDetectionPrompt();
    ...
    private String resolveLanguage(String requestLang, UserProfile profile) {
        if (requestLang != null && !requestLang.isBlank()) {
            return requestLang.equalsIgnoreCase("uk") ? "Ukrainian" : "English";
        }
